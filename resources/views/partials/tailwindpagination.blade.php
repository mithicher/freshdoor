            	...
            	@if (is_array($element))
                	@foreach ($element as $page => $url)
						@if ($page == $paginator->currentPage())
							<span class="text-blue-500 border-blue-500 -mt-px inline border-t-2 border-transparent px-4 py-3 no-underline inline-flex items-center">{{ $page }}</span>
						@else
							<a href="{{ $url }}" class="text-gray-700 -mt-px inline border-t-2 border-transparent px-4 py-3 no-underline inline-flex items-center">{{ $page }}</a>
						@endif
					@endforeach
            	@endif
			@endforeach
